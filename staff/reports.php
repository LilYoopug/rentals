...
function staff_report_build_table_rows($primary_section, $borrowings, $returns, $inventory)
{
    ...
    foreach ($borrowings as $row) {
        ...
        $rows[] = [
            ...
            'positive' => in_array($status, ['disetujui', 'aktif', 'selesai'], true),
        ];
    }
...
<a href="reports.php" class="nav-item <?= (isset($staff_active_section) && $staff_active_section === 'reports') ? 'nav-item-active' : 'text-neutral-400' ?> flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-all hover:bg-white/5" data-section="reports">
  <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <script>
      (function () {
        ...
        function buildSummary(filteredBorrowings) {
          ...
          const approvedRequests = filteredBorrowings.filter((row) => ['disetujui', 'aktif', 'selesai'].includes(row.status)).length;
          const pendingRequests = filteredBorrowings.filter((row) => ['menunggu', 'mendatang'].includes(row.status)).length;
        ...
        function buildTableRows(primarySection, filteredBorrowings, filteredReturns) {
          ...
          return filteredBorrowings.slice(0, 8).map((row) => ({
            ...
            value: row.id || '-',
            change: statusLabel(row.status),
            positive: ['disetujui', 'aktif', 'selesai'].includes(row.status)
          }));
        }
