...
$staff_active_section = 'approve-borrowings';
$staff_active_href = 'borrowings.php';
...
<html lang="en">
  <head>
    ...
    <title>LensCraft - Peminjaman</title>
    ...
    <style>
      ...
      .table-skeleton-card {
        display: grid;
        grid-template-columns: minmax(0, 1.15fr) minmax(0, 1fr) minmax(0, 1.2fr) minmax(0, 1.1fr) minmax(0, 0.9fr) minmax(0, 0.9fr) minmax(0, 0.85fr);
        gap: 1rem;
        align-items: center;
        width: 100%;
      }
    ...
    <nav class="fixed top-0 left-0 right-0 z-50 nav-blur border-b border-neutral-800 h-16">
      <div class="flex items-center justify-between h-full px-6">
        ...
        <div class="flex items-center gap-4">
          ...
          <span class="hidden md:inline-block text-sm text-neutral-500 border-l border-neutral-800 pl-4">Peminjaman</span>
        </div>
    ...
    <main class="lg:ml-64 pt-16 min-h-screen">
      <div class="p-6 md:p-8">
        ...
        <div id="content-area">
          ...
          <!-- Ringkasan Section -->
          <section id="approve-borrowings" class="content-section">
            <div class="mb-8">
              ...
              <div class="flex flex-row items-center gap-4">
                <div class="relative flex-shrink-0">
                  ...
                  <div id="borrowings-filter-dropdown" class="absolute left-0 top-full mt-2 w-80 bg-neutral-900 border border-neutral-800 rounded-xl shadow-xl z-20 hidden p-4">
                    <div class="space-y-4">
                      <div>
                        <label class="block text-xs font-medium text-neutral-300 mb-2">Status Peminjaman</label>
...
// Close mobile sidebar after selection
      ...
      renderTableLoadingState(document.getElementById('borrowings-table-body'), 7, 5);
      ...
      function renderBorrowingsTable() {
        ...
        if (pageBorrowings.length === 0) {
          renderTableEmptyState(tbody, 7, {
            ...
            isFiltered: borrowings.length > 0
          });
        ...
        tbody.innerHTML = pageBorrowings.map((borrowing) => {
          const statusBadge = `<span class="badge ${getStatusBadgeClass(borrowing.status)}">${getStatusDisplayLabel(borrowing.status)}</span>`;
          const actionButtons = `
            <button onclick="viewBorrowing('${borrowing.id}')" class="p-2 text-neutral-400 hover:text-white hover:bg-neutral-800 rounded transition-colors" title="Lihat Detail">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
              </svg>
          ...
          return `
                ...
                <div class="card-hover group">
                  <div class="flex items-start justify-between gap-3 mb-3">
                    <div class="flex items-center gap-3 min-w-0 flex-1">
                      <div class="w-12 h-12 bg-neutral-800 rounded-lg overflow-hidden flex-shrink-0">
                        <img src="${escapeHtml(borrowing.image || '../images/gear-placeholder.svg')}" alt="${escapeHtml(borrowing.equipment)}" class="w-full h-full object-cover" onerror="this.src='../images/gear-placeholder.svg'">
                      </div>
                      <div class="min-w-0">
                        <h4 class="text-sm font-semibold text-white truncate mobile-name-ellipsis mobile-card-title-ellipsis">${escapeHtml(borrowing.equipment)}</h4>
                        <p class="text-xs text-neutral-400 mobile-name-ellipsis">${escapeHtml(borrowing.customer)}</p>
                        <p class="text-xs text-neutral-500 mt-0.5">${escapeHtml(borrowing.id)}</p>
                      </div>
                    </div>
                  ...
                  <div class="space-y-2 text-xs mb-3">
                    <div class="flex justify-between gap-3">
                      ...
                      <span class="text-neutral-200 text-right mobile-name-ellipsis">${escapeHtml(borrowing.customer)}</span>
                    </div>
                    <div class="flex justify-between gap-3">
                      <span class="text-neutral-500">Periode</span>
                      <span class="text-neutral-200 text-right">${formatDate(borrowing.startDate)} - ${formatDate(borrowing.endDate)}</span>
                    </div>
                    <div class="flex justify-between gap-3">
                      <span class="text-neutral-500">Durasi</span>
                      <span class="text-neutral-200 text-right">${borrowing.days} day${borrowing.days > 1 ? 's' : ''}</span>
                    </div>
                    <div class="flex justify-between gap-3">
                      <span class="text-neutral-500">Total</span>
                      <span class="text-white font-semibold text-right">${window.formatCurrencyIDR(borrowing.amount)}</span>
                    </div>
                  </div>
            ...
            <tr class="hidden sm:table-row table-row-hover transition-colors">
              <td class="px-6 py-4">
                <span class="text-sm font-mono font-medium text-white">${escapeHtml(borrowing.id)}</span>
              </td>
              <td class="px-6 py-4">
                <div>
              ...
              <td class="px-6 py-4">
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-sm text-neutral-300 truncate max-w-[150px]">${escapeHtml(borrowing.equipment)}</span>
                </div>
              </td>
              <td class="px-6 py-4">
                <div class="text-sm">
                  <p class="text-neutral-300">${formatDate(borrowing.startDate)} - ${formatDate(borrowing.endDate)}</p>
                  <p class="text-xs text-neutral-500">${borrowing.days} day${borrowing.days > 1 ? 's' : ''}</p>
                </div>
              </td>
              <td class="px-6 py-4 text-sm font-semibold text-white">${window.formatCurrencyIDR(borrowing.amount)}</td>
              <td class="px-6 py-4">
                ${statusBadge}
              </td>
        ...
        document.getElementById('borrowings-shown').textContent = pageBorrowings.length;
        document.getElementById('borrowings-total').textContent = filteredBorrowings.length;
      ...
      function filterBorrowings() {
        const searchFilter = document.getElementById('borrowings-search')?.value.toLowerCase() || '';
        ...
        filteredBorrowings = borrowings.filter((borrowing) => {
          const matchesSearch = !searchFilter ||
            borrowing.id.toLowerCase().includes(searchFilter) ||
      ...
      document.addEventListener('DOMContentLoaded', () => {
        const borrowingsSearch = document.getElementById('borrowings-search');
        const borrowingsStatusFilter = document.getElementById('borrowings-status-filter');
        ...
        if (borrowingsSearch) borrowingsSearch.addEventListener('input', filterBorrowings);
        if (borrowingsStatusFilter) borrowingsStatusFilter.addEventListener('change', filterBorrowings);
        ...
        document.addEventListener('click', (e) => {
          if (borrowingsFilterBtn && borrowingsFilterDropdown && !borrowingsFilterBtn.contains(e.target) && !borrowingsFilterDropdown.contains(e.target)) {
  ...
  <script>
      ...
      function viewBorrowing(id) {
        ...
        openStaffDetailModal({
          ...
          badges: [
            `Status: ${getStatusDisplayLabel(borrowing.status)}`,
          ...
          rows: [
            ...
            { label: 'Total', value: `${window.formatCurrencyIDR(borrowing.amount)}` }
          ]
        });
      }
      ...
      function approveBorrowing(id) {
        ...
        openStaffActionModal('Setujui Peminjaman', 'Anda akan menyetujui peminjaman ini. Pastikan semua detail sudah benar sebelum melanjutkan.', function () {
          ...
          rentalCode: borrowing.id,
          name: borrowing.equipment,
          brand: borrowing.brand || 'LensCraft',
          category: borrowing.category || 'equipment',
          image: borrowing.image,
          customer: borrowing.customer,
          period: formatDate(borrowing.startDate) + ' - ' + formatDate(borrowing.endDate),
          duration: (borrowing.days || 0) + ' hari',
          delivery: borrowing.deliveryMethod === 'diantar' ? 'Diantar' : 'Ambil Sendiri',
          price: formatCurrency(borrowing.dailyRate || 0),
          discount: (borrowing.discount || 0) + '%',
          total: formatCurrency(borrowing.amount || 0),
          deliveryFee: borrowing.deliveryMethod === 'diantar' && borrowing.deliveryFee ? formatCurrency(borrowing.deliveryFee) : null
        } : null, function () {
