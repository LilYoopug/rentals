...
function tracked_ids(array $rows): array {
    $ids = [];
    foreach ($rows as $row) {
        $id = (string) ($row["id"] ?? "");
...
$admin = tracked_ids(extract_array(file_get_contents($argv[1]), "borrowingsData"));
$staff = tracked_ids(extract_array(file_get_contents($argv[2]), "borrowings"));

if ($admin !== ["RENT-SYNC-COMPLETED", "RENT-SYNC-REJECTED"]) {
    fwrite(STDERR, "Unexpected admin tracked borrowings: " . implode(", ", $admin) . "\n");
    exit(1);
}

if ($staff !== $admin) {
    fwrite(STDERR, "Staff borrowings not synced with admin: " . implode(", ", $staff) . "\n");
    exit(1);
}
