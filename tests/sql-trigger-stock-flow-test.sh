...
after_approved_stock="$(mysql_test -N -B lenscraft -e "SELECT stock_available FROM products WHERE id = 3")"
if [[ "${after_approved_stock}" != "${after_pending_stock}" ]]; then
  echo "Expected disetujui rental to keep stock reserved, got ${after_approved_stock:-<empty>} from ${after_pending_stock:-<empty>}"
  exit 1
fi
