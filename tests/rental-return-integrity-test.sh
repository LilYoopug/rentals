...
VALUES ('Other User', 'other@example.com', 'other', '${password_hash}', 'pelanggan', 'aktif', NOW());
...
VALUES ('RENT-2026-OTHER', @other_id, 1, CURDATE(), DATE_ADD(CURDATE(), INTERVAL 1 DAY), 2, 89.00, 0, 'ambil_sendiri', 0.00, 178.00, 'aktif', NOW(), NOW());
SQL
...
if [[ "${other_return_response}" != *'"success":false'* ]]; then
  echo "Expected returning another pelanggan's rental to fail, got: ${other_return_response}"
  exit 1
fi
...
rental_response="$(curl -sS -b "${cookie_file}" -X POST -d "csrf_token=${csrf_token}&product_id=1&start_date=2026-04-10&end_date=2026-04-11&delivery_method=ambil_sendiri" "${TEST_BASE_URL}/process/rental-create-process.php")"
