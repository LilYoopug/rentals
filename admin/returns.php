...
$admin_active_section = 'returns';
$admin_active_href = 'returns.php';
...
$admin_return_rows = get_return_tracking_rows();
...
<html lang="en">
  <head>
    ...
    <style>
      ...
      .admin-js.admin-page-loading #returns {
        ...
        <div id="content-area">
          <section id="admin-page-skeleton" aria-hidden="true">
            <div class="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              <div class="space-y-3">
                <div class="skeleton-shell h-10 w-64 rounded-2xl"></div>
            ...
            <div class="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 space-y-4">
              <?php for ($admin_returns_skeleton = 0; $admin_returns_skeleton < 5; $admin_returns_skeleton++): ?>
    ...
    <script>
      ...
      function renderPeminjamanTable() {
        ...
        tbody.innerHTML = pageItems.map(trx => {
          const statusBadge = `<span class="badge ${getPeminjamanStatusBadgeClass(trx.status)}">${getStatusDisplayLabel(trx.status)}</span>`;
          const actionButtons = `
      ...
      function renderPengembalianTable() {
        ...
        tbody.innerHTML = pageItems.map(trx => {
          const statusBadge = `<span class="badge ${getPengembalianStatusBadgeClass(trx.status)}">${getStatusDisplayLabel(trx.status)}</span>`;
          const editAction = trx.status === 'borrowed'
            ? `
            <button onclick="convertToPengembalian('${trx.id}')" class="p-2 text-neutral-400 hover:text-green-400 hover:bg-neutral-800 rounded transition-colors" title="Buat Pengembalian">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          return `
              ...
              <td colspan="6" class="p-4">
                <div class="card-hover group">
                  ...
                  <div class="space-y-2 text-xs mb-3">
                    ...
                    <div class="flex justify-between gap-3">
                      <span class="text-neutral-500">${trx.status === 'borrowed' ? 'Target Return' : 'Tanggal Return'}</span>
                    ...
                    <div class="flex justify-between gap-3">
                      ...
                      <span class="text-white text-right font-semibold">${window.formatCurrencyIDR(trx.fineAmount || 0)}</span>
                    </div>
                  </div>
            ...
            <tr class="hidden sm:table-row table-row-hover transition-colors">
              ...
              <td class="px-6 py-4 text-sm font-semibold text-white">${window.formatCurrencyIDR(trx.fineAmount || 0)}</td>
              <td class="px-6 py-4 text-right">
                <div class="flex items-center justify-end gap-2">
                  ${actionButtons}
                </div>
  ...
  <script>
      ...
      function viewPengembalian(trxId) {
        ...
        openAdminDetailModal({
          ...
          kicker: trx.status === 'borrowed' ? 'Item Dipinjam' : 'Pengembalian',
          title: escapeHtml(trx.equipment),
          ...
          rows: [
            { label: trx.status === 'borrowed' ? 'Jatuh Tempo' : 'Tanggal', value: escapeHtml(formatDate(trx.returnDate)) },
            { label: 'Denda', value: escapeHtml(window.formatCurrencyIDR(trx.fineAmount || 0)) }
          ],
          note: escapeHtml(trx.notes || (trx.status === 'borrowed' ? 'Belum ada permintaan pengembalian.' : 'Tidak ada catatan tambahan.'))
        });
      }
    ...
    <script>
document.addEventListener('DOMContentLoaded', function () {
  if (typeof renderPengembalianTable === 'function') {
    renderPengembalianTable();
  }
